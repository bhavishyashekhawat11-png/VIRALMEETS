...
export function UpgradeModal({ onManageSubscription }: { onManageSubscription?: () => void }) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        >
          {/* Top Banner */}
          <div className="h-32 bg-gradient-to-br from-rose-600 to-rose-400 relative overflow-hidden flex items-center justify-center">
            <div className="absolute inset-0 opacity-20">
              <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white via-transparent to-transparent" />
            </div>
            <Rocket className="w-16 h-16 text-white/20 absolute -right-4 -bottom-4 rotate-12" />
          ...
          <div className="p-8 pt-6">
            ...
            <div className="space-y-4">
              <button
                onClick={upgrade}
                className="w-full bg-rose-600 hover:bg-rose-500 text-white font-black py-4 rounded-2xl shadow-xl shadow-rose-900/40 transition-all active:scale-[0.98] relative group overflow-hidden"
              >
              ...
              {onManageSubscription && (
                <button
                  ...
                  className="w-full text-rose-500/60 hover:text-rose-500 text-[10px] font-black uppercase tracking-[0.15em] transition-colors pt-2"
                >
          ...
          <button 
            onClick={() => setShowUpgradeModal(false)}
            className="absolute top-4 right-4 w-8 h-8 bg-black/20 hover:bg-black/40 backdrop-blur-md rounded-full flex items-center justify-center text-white/70 hover:text-white transition-all"
          >
            <X className="w-4 h-4" />
          </button>
        </motion.div>
